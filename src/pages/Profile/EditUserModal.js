...
import {
    Modal, ModalBody, ModalHeader, Spinner, Col, Nav, NavItem, NavLink, Row, TabContent, TabPane, Label, DropdownMenu, DropdownItem, Dropdown, DropdownToggle, Button,
} from "reactstrap";
...
import { countriesList, statesList, citiesList, updateUserProfile, updateUserProfileAdditional, updateUserProfilePassword } from "../../slices/thunks";
    ...
    const onSubmit = (e) => {
        ...
        if (activeTab == 1) {
            if (formData?.dob) {
                ...
                const returning = dispatch(updateUserProfile(data))
                returning.then(res => {
        ...
        if (activeTab == 2) {
            ...
            const returning = dispatch(updateUserProfileAdditional(data))
            returning.then(res => {
                if (res && e.nativeEvent.submitter.id == 'save_next') {
                    changeTab()
                    toggleTab(activeTab + 1);
        ...
        if (activeTab == 3) {
            if (formData?.password && formData?.password?.length > 7 && (formData?.password == formData?.password_confirmation)) {
                ...
                const returning = dispatch(updateUserProfilePassword(data))
                returning.then(res => {
                    if (res && e.nativeEvent.submitter.id == 'save_next') {
                        onCloseClick()
                    }
